unit uViaCEPContoller;

interface

uses IdHTTP, IdSSLOpenSSL, uIViaCEP, uViaCEPModel;
...
type
  ...
  private
    ...
    /// </returns>
    function Get(const ACep: string): TViaCEPModel;
    /// <summary>
...
function TViaCEP.Get(const ACep: string): TViaCEPModel;
const
...
begin
  ...
  try
    ...
    if (FIdHTTP.ResponseCode = 200) and (not (LResponse.DataString).Equals(INVALID_CEP)) then
      Result := TJson.JsonToObject<TViaCEPModel>(UTF8ToString(PAnsiChar(AnsiString(LResponse.DataString))));
  finally
    LResponse.Free;
  end;
end;
